...
# it will take from the directory the file name image.png, then based on the name, use Regex to separate the code students, students name, code answer.
...
def One_Student_Result(sheetName):
    ...
    for i_pos in range(0, 12):
        ...
        # Image smoothing and noise filtering
        edge_doc = cv2.Canny(blur_doc, 75, 200)
        # draw 2-color border border, and frame border
        grade_cnt = cv2.findContours(edge_doc, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        #find the contours of the bubble that the user has filled. Specific examples are the answers that students have painted
...
def first_five_answer(sheetName):
    ...
    # give the candidate's answer, the answer of the question code and the candidate's wrong answer
    # export image
